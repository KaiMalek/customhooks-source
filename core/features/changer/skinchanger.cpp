...
void skinchanger::run() {
	...
	for (size_t i = 0; my_weapons[i] != 0xFFFFFFFF; i++) {
		...
		case WEAPON_KNIFE_BAYONET:
			switch (variables::config::knife_skin)
			{
			case 0:
...
void Hooked_RecvProxy_Viewmodel(c_recv_proxy_data* pData, void* pStruct, void* pOut)
{
	...
	if (csgo::local_player)
	{
		...
		{
			if (variables::config::knife_model == 1)
				pData->value.m_int = iBayonet;
			else if (variables::config::knife_model == 2)
				pData->value.m_int = iFlip;
			else if (variables::config::knife_model == 3)
				pData->value.m_int = iGut;
			else if (variables::config::knife_model == 4)
				pData->value.m_int = iKarambit;
			else if (variables::config::knife_model == 5)
				pData->value.m_int = iM9Bayonet;
			else if (variables::config::knife_model == 6)
				pData->value.m_int = iHuntsman;
			else if (variables::config::knife_model == 7)
				pData->value.m_int = iFalchion;
			else if (variables::config::knife_model == 8)
				pData->value.m_int = iBowie;
			else if (variables::config::knife_model == 9)
				pData->value.m_int = iButterfly;
			else if (variables::config::knife_model == 10)
				pData->value.m_int = iDagger;
			else if (variables::config::knife_model == 11)
				pData->value.m_int = iUrsus;
			else if (variables::config::knife_model == 12)
				pData->value.m_int = iNavaja;
			else if (variables::config::knife_model == 13)
				pData->value.m_int = iStiletto;
			else if (variables::config::knife_model == 14)
				pData->value.m_int = iTalon;
			else if (variables::config::knife_model == 15)
				pData->value.m_int = iNomad;
			else if (variables::config::knife_model == 16)
				pData->value.m_int = iSkeleton;
			else if (variables::config::knife_model == 17)
				pData->value.m_int = iSurvival;
			else if (variables::config::knife_model == 18)
				pData->value.m_int = iParacord;
			else if (variables::config::knife_model == 19)
				pData->value.m_int = iClassic;
...
void skinchanger::animfix_hook()
{
	for (c_client_class* pClass = interfaces::client->get_client_classes(); pClass; pClass = pClass->next_ptr) {
		if (!strcmp(pClass->network_name, "CBaseViewModel")) {
			// search for m_nModelIndex property
