...
static auto make_glove(int entry, int serial) -> attributable_item_t* {
	...
	assert(glove); {
		static auto set_abs_origin_addr = utilities::pattern_scan2(GetModuleHandleA(xorstr_("client.dll")), xorstr_("55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8"));
...
void glovechanger::run() {
	...
	if (variables::config::glove_model == 0)
		return;
	...
	if (variables::config::glove_model > 0) {
		...
		if (glove)
		{
			...
			case 0:
				break;
			case 1:
				apply_glove_skin(glove, GLOVE_STUDDED_BLOODHOUND, get_glove_skin(GLOVE_STUDDED_BLOODHOUND, variables::config::bloodhound_skin), index_blood, 3, g_wear);
				break;
			case 2:
				apply_glove_skin(glove, GLOVE_SPORTY, get_glove_skin(GLOVE_SPORTY, variables::config::sport_skin), index_sport, 3, g_wear);
				break;
			case 3:
				apply_glove_skin(glove, GLOVE_SLICK, get_glove_skin(GLOVE_SLICK, variables::config::driver_skin), index_slick, 3, g_wear);
				break;
			case 4:
				apply_glove_skin(glove, GLOVE_LEATHER_WRAP, get_glove_skin(GLOVE_LEATHER_WRAP, variables::config::handwrap_skin), index_leath, 3, g_wear);
				break;
			case 5:
				apply_glove_skin(glove, GLOVE_MOTORCYCLE, get_glove_skin(GLOVE_MOTORCYCLE, variables::config::moto_skin), index_moto, 3, g_wear);
				break;
			case 6:
				apply_glove_skin(glove, GLOVE_SPECIALIST, get_glove_skin(GLOVE_SPECIALIST, variables::config::specialist_skin), index_speci, 3, g_wear);
				break;
			case 7:
				apply_glove_skin(glove, GLOVE_HYDRA, get_glove_skin(GLOVE_HYDRA, variables::config::hydra_skin), index_hydra, 3, g_wear);
				break;
			}
